...
These stories begin after v1.0 ships.  They represent RPG Watcher's evolution from a Discord-first bot into a universal session companion that works for any group, on any voice platform, without requiring Discord at all.
...
**I want to** run `rpgwatcher process session.wav` on an existing audio file and receive a structured session note,
**so that** I can retroactively generate notes for every prior session I recorded but never processed.

**Acceptance criteria:**
- [ ] `rpgwatcher process <file>` CLI command accepts a `.wav`, `.mp3`, or `.ogg` file path
...
**Acceptance criteria:**
- [ ] `rpgwatcher process --tracks "player1.wav:Theron Ashveil" "player2.wav:Mira" "gm.wav:Narrator"` accepts multiple labelled files
...
**As a** developer,
**I want to** implement the `AudioSource` protocol in my own module and register it with RPG Watcher,
...
- [ ] A third-party package can register an `AudioSource` implementation via a `rpgwatcher.audio_sources` entry point in `pyproject.toml`
...
**so that** I can integrate RPG Watcher with my group's existing tooling.

**Acceptance criteria:**
- [ ] `OutputAdapter` is a `typing.Protocol` with: `async write(note: SessionNote) -> str` (returns a URL or path to the written note)
- [ ] Third-party adapters register via a `rpgwatcher.output_adapters` entry point
...
**so that** I have a single place to manage and review everything RPG Watcher has captured.
...
**so that** I can use RPG Watcher for sessions on other platforms where I'm not using Discord.
...
**As a** GM (Sam),
**I want to** open the RPG Watcher dashboard on my tablet while my bot runs on my home server,
...
**I want to** start RPG Watcher's loopback capture before the Zoom call and have it record everything played through my speakers,
**so that** I get the same session notes my Discord-using friends get, with no extra software installed.

**Acceptance criteria:**
- [ ] RPG Watcher detects available loopback devices (WASAPI loopback on Windows, BlackHole on macOS if installed, PulseAudio monitor on Linux) and lists them in the source selector
...
**As a** GM (River),
**I want to** place my laptop in the middle of the table, click "Start Recording", and have RPG Watcher capture the whole session from the room mic,
...
**so that** I can process sessions that were captured by another tool rather than by RPG Watcher directly.
...
**As a** GM (Morgan),
**I want to** import a Zoom local recording folder and have RPG Watcher process the audio tracks into a session note,
**so that** even platform-recorded meetings from before RPG Watcher was installed can become searchable notes.

**Acceptance criteria:**
- [ ] Zoom local recordings include `audio_only.m4a` (mixed) and optionally per-participant `.m4a` in a subfolder; RPG Watcher uses per-participant files when available
...
**As a** player (Alex or River),
**I want to** record a short voice sample so RPG Watcher can recognise my voice in future sessions,
...
**As a** GM (River),
**I want to** have RPG Watcher write plain Markdown files to a directory on my computer rather than requiring an Obsidian vault,
**so that** I can use RPG Watcher without adopting a new note-taking application.

**Acceptance criteria:**
- [ ] `OUTPUT_ADAPTERS=markdown` writes `.md` files to `NOTES_DIR` (default: `~/RPG Watcher/notes/`)
...
**As a** developer,
**I want to** configure a webhook URL so that RPG Watcher POSTs a JSON payload after each session,
**so that** I can integrate session notes into any custom downstream system without adding code to RPG Watcher.
...
- [ ] `WEBHOOK_SECRET` env var, if set, adds an `X-RPG Watcher-Signature` HMAC-SHA256 header for receiver verification
...
### US-119 — Install RPG Watcher on Windows with no technical knowledge

**As a** non-Discord user (River),
**I want to** download a `.exe` installer and have RPG Watcher running with a browser dashboard in 5 minutes,
...
- [ ] Uninstaller cleanly removes all RPG Watcher files but prompts to keep notes and session data
...
### US-121 — Run RPG Watcher completely offline after initial setup

**As a** GM (River),
**I want to** use RPG Watcher at a cabin with no internet where we play board games and TTRPGs,
...
- [ ] RPG Watcher checks for updates only on startup and only if the user opts in; update checks never block startup
...
**I want to** manage three separate ongoing campaigns — each with different players, character registries, and vocabularies — from a single RPG Watcher instance,
...
**I want to** be notified in the Web UI when a new version of RPG Watcher is available and update it from there,
**so that** I benefit from improvements and fixes without needing to use a terminal or package manager.

**Acceptance criteria:**
- [ ] On startup, RPG Watcher checks GitHub Releases for a newer version (opt-in; default on; can disable via `AUTO_UPDATE_CHECK=false`)
...
**I want to** speak my session journal out loud and have RPG Watcher transcribe and format it into a structured note,
